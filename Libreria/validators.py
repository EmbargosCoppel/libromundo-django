...
def validar_no_consecutivos(value):
    # es par si hay caracteres consecutivos
    if re.search(r'(.)\1\1', value):
        raise ValidationError(
            'El nombre de usuario no puede tener 3 o más caracteres idénticos consecutivos.'
        )

class ComplexPasswordValidator: #Aquí definimos la clase para validar la contraseña con los requisitos que queremos
    def validate(self, password, user=None):
    # Verificamos que cumpla con la que queremos para la contraseña
        if not re.search(r'[!@#$%^&*()]', password):
         raise ValidationError("Falta carácter especial.")
